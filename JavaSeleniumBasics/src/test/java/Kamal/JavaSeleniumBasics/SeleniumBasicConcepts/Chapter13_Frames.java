package Kamal.JavaSeleniumBasics.SeleniumBasicConcepts;

import java.time.Duration;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.edge.EdgeDriver;
import org.openqa.selenium.support.ui.Select;
import org.testng.annotations.BeforeTest;
import org.testng.annotations.Test;

import io.github.bonigarcia.wdm.WebDriverManager;

public class Chapter13_Frames {
	WebDriver driver;

	@BeforeTest
	public void beforeTest() {
		WebDriverManager.edgedriver().setup();
		driver = new EdgeDriver();
		driver.manage().window().maximize(); // Maximize the browser window
		driver.get("https://www.letskodeit.com/practice"); // Open Bing website
		// Implicit wait
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
	}
  @Test
  public void handleIFrame() {
	  System.out.println("Title of the page: " + driver.getTitle()); // Print the title of the page
	  System.out.println("Title of the iframe page: " + driver.getTitle());
	  //Switch to iframe using id
	  WebElement  iframeElement = driver.findElement(By.id("courses-iframe"));
	  driver.switchTo().frame(iframeElement);
	  
	  //Now we are inside the iframe, we can interact with elements inside the iframe
	  Select s = new Select(driver.findElement(By.name("categories")));
	  s.selectByVisibleText("Test Automation");
	  System.out.println("Selected 'Test Automation' from the dropdown inside the iframe.");
	  
	 
	  
	  //Switch back to main content
	  driver.switchTo().defaultContent();
	  System.out.println("Switched back to main content.");
	  System.out.println("Title of the main page: " + driver.getTitle());
  }
}
